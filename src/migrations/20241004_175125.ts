...
export async function up({ payload, req }: MigrateUpArgs): Promise<void> {
  await payload.db.drizzle.execute(sql`
   DO $$ BEGIN
   CREATE TYPE "public"."enum_payload_jobs_log_task_slug" AS ENUM('inline', 'createPost');
  EXCEPTION
  ...
  DO $$ BEGIN
   CREATE TYPE "public"."enum_payload_jobs_task_slug" AS ENUM('inline', 'createPost');
  EXCEPTION
   WHEN duplicate_object THEN null;
  END $$;
  ...
  CREATE TABLE IF NOT EXISTS "payload_jobs_log" (
  	...
  	"taskSlug" "enum_payload_jobs_log_task_slug" NOT NULL,
  ...
  CREATE TABLE IF NOT EXISTS "payload_jobs" (
  	...
  	"has_error" boolean,
  	"error" jsonb,
  	"workflowSlug" "enum_payload_jobs_workflow_slug",
  	"taskSlug" "enum_payload_jobs_task_slug",
  	"queue" "enum_payload_jobs_queue" DEFAULT 'default',
  	"wait_until" timestamp(3) with time zone,
  	"processing" boolean DEFAULT false,
  	"seen_by_worker" boolean DEFAULT false,
  	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
  	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL
  );
  ...
  CREATE INDEX IF NOT EXISTS "payload_jobs_workflow_slug_idx" ON "payload_jobs" USING btree ("workflowSlug");
  CREATE INDEX IF NOT EXISTS "payload_jobs_task_slug_idx" ON "payload_jobs" USING btree ("taskSlug");
